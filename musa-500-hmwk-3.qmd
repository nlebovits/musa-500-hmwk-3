...
Additionally, OLS regression relies on continuous variables. In this assignment,
we are dealing with binary variables as well as continuous variables, so regular
OLS regression is inappropriate.

### Logistic Regression

#### Assumptions of Logistic Regression

Instead of OLS regression, we will use an approach known as logistic regression.
The assumptions of logistic regression are similar to the assumptions in OLS regression,
but not identical. In logistic regression:

 1. The dependent variable must be binary
 2. Observations must be independent
 3. There must not be multicollinearity

We will also need a larger sample size than in OLS because in logistic regression
we rely on maximum likelihood estimation. Logistic regression also differs from OLS
in that:
...
In order to understand logistic regression, we must first review the concept of
*odds*.

Odds are similar to probability. However, while probability may be calculated
as $\frac{\text{\# desirable outcomes}}{\text{\# possible outcomes}}$, odds are 
calculated as $\frac{\text{\# desirable outcomes}}{\text{\# undesirable outcomes}}$.

The log odds that $Y = 1$ (or the *logit*) are simply $ln(Odds(Y = 1))$. In other words,
if $Y$ is our dependent variable and $X$ is our independent variable, a 1-unit
increase in $X$ means that the odds that $Y=1$ go up by $e^\text{ln(Odds(Y = 1))}$.

The *odds ratio* is the ratio of the odds of one outcome for our binary dependent
variable versus the other possible outcome, or $\frac{\text{Odds(Y=1)}}{\text{Odds(Y=0)}}$. It can also be calculated by exponentiating
the coefficient of the independent variable in question. The odds ratio indicates
how much more likely we are to see $Y=1$ than $Y=0$, e.g., an odds ratio of 3
means that it is 3 times more likely that $Y=1$ than that $Y=0$.^[Sperandei S. Understanding logistic regression analysis. Biochem Med (Zagreb). 2014 Feb 15;24(1):12-8. doi: 10.11613/BM.2014.003. PMID: 24627710; PMCID: PMC3936971.] Rather than looking at the estimated $\beta$ coefficients, most statisticians prefer to look at odds ratios
...
Instead, we can use the Akaike Information Criterion to compare models. The AIC estimates prediction error, and therefore the quality of our model, and includes a penality that "is an increasing function of the number of estimated parameters".^[https://en.wikipedia.org/wiki/Akaike_information_criterion]  When comparing multiple models, "the one with the minimum AIC value assures a good balance of goodness of fit and complexity".^[https://www.sciencedirect.com/topics/social-sciences/akaike-information-criterion]
...
![Figure 1: ROC Curve Example](File:Roccurves.png)](https://en.wikipedia.org/wiki/Receiver_operating_characteristic#/media)
