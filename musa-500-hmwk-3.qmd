...
The question is then whether the number of drunk drivers should be considered small sample. The answer is uncertain. According to Table 2-1-1, we have 2,485 observations for drivers who were drunk at the time of incidents, versus 40,879 observations for drivers who were not drunk. Although 2,485 seems like a reasonably sized group, it only take 0.057% of all observations. Therefore, there might be a need to adopt Paul Allison's methods for modeling rare events instead of just using logistic regression. 
